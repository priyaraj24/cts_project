...
public class BookingDaoImpl implements BookingDao {
	...
	public List<BookFlight> fetchHistory(int id) {
		try {
			...
			list = SessionFactory.getCurrentSession().createQuery("FROM BookFlight WHERE status='TRAVELLED' AND userId= "+id)
	...
	public List<BookFlight> fetchBookings(int id) {
		try {
			List<BookFlight> list = new ArrayList<>();
			list = SessionFactory.getCurrentSession().createQuery("FROM BookFlight WHERE status IN ('CANCELLED','BOOKED') AND userId ="+id)
					//.setParameter("userId", id)
					.list();
			return list;
		} catch (Exception e) {
